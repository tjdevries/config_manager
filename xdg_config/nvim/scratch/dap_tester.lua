...
VariableBuffers = VariableBuffers or {}
local variable_buffers = VariableBuffers
...
function ToggleVariableExpanded()
...
local function popup()
  ...
  api.nvim_buf_set_keymap(
    ...
    "n",
    "<CR>",
    "<Cmd>lua ToggleVariableExpanded()<CR>",
    {}
  )
  api.nvim_buf_set_keymap(
    buf,
...
function M.put_in_split()
  local buf = 65
  local win = 1023

  local session = require('dap').session()
  create_variable_buffer(buf, win, session, session.current_frame.scopes)
...
vim.cmd [[nnoremap asdf :lua DapTest()<CR>]]
