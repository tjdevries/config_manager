...
log.debug("Package Path   : %s", package.path)
log.debug("LuaRocks Loaded: %s", luarocks_success)

-- TODO: Figure out environment variables for more portable config
-- local submodule_dir = os.getenv("XDG_CONFIG_HOME") .. '/submodules/'
local submodule_dir = "/home/tj/.config/submodules/"
...
local globalkeys = gears.table.join(
  ...
  -- Volume control
  awful.key({ mods.super }, "]", require("teej.volume").inc),
  awful.key({ mods.super }, "[", require("teej.volume").dec),
  awful.key({ mods.super, mods.shift }, "t", function()
    local inspect = require "teej.inspect"

    -- naughty.notify { text = inspect(titlebar) }
    require("teej.file").write("/home/tjdevries/tmp/output.txt", inspect(titlebar))
    -- titlebar:toggle()
  end)
)

clientkeys = gears.table.join(awful.key({ mods.super }, "f", function(c)
  ...
  function(c)
    ...
    --   prompt = "Testing! ",
    -- }
    dofile "/home/tjdevries/.config/awesome/test_script.lua"
  end
), awful.key(
  { mods.super, mods.shift },
  "c",
